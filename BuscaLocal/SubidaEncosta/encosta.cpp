...
int randomNumber() {
    ...
    // Defina a faixa de números aleatórios que você deseja gerar
    int min = 0;
    int max;
    if(OPC == 1) {
        max = (N*N)-1;
    ...
    // Crie uma distribuição uniforme de números inteiros nessa faixa
...
State generateRandomState(State state) {
    vvi matrix = state.matrix;
    std::set<int> numbers;
    int n,i,j;
    int max = 0;
    do {
        if(OPC == 1) { // Quebra-cabeça
            ...
            matrix[i][j] = 2;
            matrix[state.row][state.column] = 1;
        }
    } while(visitedStates.find(matrix) != visitedStates.end() and max < MAXTEST);

    State random(states.size(),state.id,calculateHeuristic(matrix,i,j),i,j,N,matrix);

    return random;
}

lli hillClimbing(State state) {
    ...
    do {
        ...
        for(int k = 0; k < 4; k++) {
            ...
            if(New.heuristic < current.heuristic) {
                next = New;
                ok = true;
            }
        }
...
int main() {
    ...
    while(std::cin >> OPC >> N) {
        ...
        do {
            if(ans != -1) {
                State Random = generateRandomState(states[ans]);
                states.push_back(Random);
            }
            ans = hillClimbing(states.back());
            numberTest++;
        } while(!isSolution(states[ans].matrix,states[ans].row, states[ans].column) and numberTest < MAXTEST);
